...
def conv_forward(x, w):
    ...
    for i in range(0, H_prime):
        for j in range(0, W_prime):
            # submatrix is x[:, :, i:i+HH, j:j+WW]
            out[:, :, i, j] = np.einsum(
                "abcd,ebcd->ea", w, x[:, :, i : i + HH, j : j + WW], optimize=True
            )
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward(dout, cache):
    ...
    w_pad = np.pad(
        w,
        ((0, 0), (0, 0), (pad_len[0], pad_len[0]), (pad_len[1], pad_len[1])),
        "constant",
        constant_values=0,
    )
            ...
            out[:, :, x1, y] = np.amax(
                np.amax(
                    x[
                        :,
                        :,
                        x1 * stride : x1 * stride + p_H,
                        y * stride : y * stride + p_W,
                    ],
                    axis=-1,
                ),
                axis=-1,
...
def max_pool_backward(dout, cache):
    ...
    for x1 in range(math.floor(H_out)):
        for y in range(math.floor(W_out)):
            ...
            temp = np.zeros(
                x[
                    :, :, x1 * stride : x1 * stride + p_H, y * stride : y * stride + p_W
                ].shape
            )
            temp = (
                x[:, :, x1 * stride : x1 * stride + p_H, y * stride : y * stride + p_W]
                == max_element
            )
            tmp_dout = dout[:, :, x1, y]
