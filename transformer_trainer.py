...
class Trainer:
    ...
    def train_log(self, loss, type):
        ...
        if (self.iter_num % self.log_interval == 0) or (
            (self.iter_num + 1) >= self.max_iters
        ):
            print(
                f"Iteration {self.iter_num + 1}/{self.max_iters}: training loss {loss.item():.4f}"
            )
            if type == "multiplication":
                Evaluator(self.dataset, self.model, "train").eval_split(self.device)
            elif type == "story":
                contexts = ["Once upon a time", "Once there was an ", "One day"]
                for i in range(3):
                    with torch.no_grad():
                        x = torch.tensor(
                            [[self.dataset.stoi[s] for s in contexts[i]]]
                        ).long()
                        y = self.model.generate(
                            x.to(self.device), 1024, sampling=True, top_k=10
                        )[0]
...
class Evaluator:
    ...
    def eval_split(self, device, print_example=0):
        ...
        loader = DataLoader(
            self.dataset, batch_size=64, num_workers=0, shuffle=True, drop_last=False
        )
        for x, _ in loader:
            B = x.size(0)
            ab = x[:, : n * 2].to(device)
            ...
            a, b = (
                self.dataset.str_to_digit(ab[:, :n], device),
                self.dataset.str_to_digit(ab[:, n : n * 2], device),
            )
            c_gt = a * b
        ...
        for a[i], b[i], c_pred[i], c_gt[i] in all_preds[:print_example]:
            print(
                f"Prediction: {a[i]} x {b[i]} = {c_pred[i]}, correct answer: {c_gt[i]}"
            )
